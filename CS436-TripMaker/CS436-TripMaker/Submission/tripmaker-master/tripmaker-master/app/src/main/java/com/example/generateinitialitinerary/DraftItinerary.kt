...
class DraftItinerary : Fragment() {
    ...
    override fun onCreateView(
        ...
        //pushes itinerary to firebase
        view.finishItineraryButton.setOnClickListener {
            val arrayList = (view.recyclerView.adapter as DraftAdapter).arrayList
            val uid = FirebaseAuth.getInstance().currentUser?.uid
            val destination = (requireActivity() as MapsActivity).destination

            var dumbyList = ArrayList<TripDumby>()
            arrayList.forEach { it ->
                dumbyList.add(it.toDumby())
            }

            FirebaseDatabase.getInstance().reference.child("$uid/$destination").setValue(dumbyList.toList()).addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Log.i("info_q", "succesful setValue")
                    requireActivity().finish()
                } else {
                    Log.i("info_q", "failed setValue")
                }
            }
        }
